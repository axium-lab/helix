...
The mappers live at `src/internal/providers/_shared/openai-shape.mappers.ts` — grouped by upstream SDK shape, not by provider. All three current adapters (OpenAI, Azure, Custom) consume them, since `AzureOpenAI` extends `OpenAI` and `custom` is OpenAI-compatible by contract. When a non-OpenAI-shape provider lands (Vertex, Anthropic), it adds a sibling file under `_shared/` (e.g. `vertex-shape.mappers.ts`) — `_shared/` hosts mappers per SDK shape, never per provider. The guard test in `tests/unit/adapter-cast-removal.test.ts` iterates over every OpenAI-shape adapter to keep regression coverage uniform.
