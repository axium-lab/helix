...
Governs the test coverage required for this change. Satisfies PR3 ("tests with openai, azure, custom") for the three in-scope providers. Vertex coverage is explicitly waived for this change and will be delivered by `helix-vertex-provider`. Uses Vitest as the test runner and `vi.mock("openai")` for SDK-level mocking in the unit tier — no HTTP-level interception.
...
- GIVEN `vi.mock("openai")` returns a valid `HelixResponse`-shaped payload via `mockResolvedValue`
...
- GIVEN the SDK mock rejects via `mockRejectedValue`
...
- GIVEN `vi.mock("openai")` returns a valid `FileObject`-shaped payload via `mockResolvedValue`
...
- GIVEN `vi.mock("openai")` returns a models list payload with two entries via `mockResolvedValue`
...
- GIVEN `vi.mock("openai")` resolves the models list call via `mockResolvedValue`
...
- GIVEN `vi.mock("openai")` rejects the models list call via `mockRejectedValue`
- WHEN `helix.test()` is called on the OpenAI adapter
